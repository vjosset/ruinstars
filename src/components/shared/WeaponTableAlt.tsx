...
export default function WeaponTableAlt({ 
  weapons,
  MSK,
  RSK,
  selectedGearIds = [],
      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="text-muted border-b border-border">
            ...
            <th className="text-center w-1/8"><h6>ATT / SKL</h6></th>
          </tr>
        </thead>
        <tbody>
          {weapons.map((gear) => (
            <tr key={gear.gearId}>
              ...
              <td className="text-center stat py-0.5"><span className="stat text-main leading-none px-2 bg-black border-border border">{gear.ATT ?? '-'} / {gear.TYP === 'M' ? MSK : RSK}</span></td>
            </tr>
          ))}
        </tbody>
      </table>
